...
function Listing({ product }) {
  ...
  const { parent, id, index } = router.query;
  if (parent) {
    var { data: metaData } = useGetMetaDetailBySlug(parent);
  }
  ...
  return (
    <>
      ...
      <ExploreWideRange />
      <Lists />
      <MetaContent paramKey={parent} paramField={"slug"} />
      <Testimonial />
      <NewsLetter />
      <Footer />
      <Disclaimer />
...
export async function getServerSideProps(context) {
  try {
    ...
    if (res.status === 200) {
      ...
      console.log({ dfdfdfdfdrf: findSelectedCat })
      if (findSelectedCat) {
        const res1 = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/getAllProducts?category_slug=${context?.params?.parent}&sub_category_slug=${context?.params?.parent}&parent_slug=${findSelectedCat?.slug_key}`)
