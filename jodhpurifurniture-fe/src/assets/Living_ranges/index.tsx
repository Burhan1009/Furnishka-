...
const Living_ranges = () => {
  ...
  return (
    ...
    <head>
        <SeoHeader
          title={getCetegory[1]?.meta_title}
          description={getCetegory[1]?.meta_description}
        />
      </head>
      <section className="sec  section-2  row-remove">
        <div className="container2">
          <label className="top-xy range-other"> Living Furniture </label>
          ...
          <div className="row  boxes-padding margin-bottom-xy ">
            ...
            <Slider {...settings}>
              {getCetegory &&
                getCetegory[1]?.child_categories &&
                getCetegory[1]?.child_categories.map((item) => {
                  return item.child_categories?.map((val) => {
                    return (
                      <div className="col-4 col-lg-2 boxes-padding h6 text-hover-image">
                        <Link
                          href={`/${getCetegory[1]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                          as={`/${getCetegory[1]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                        >
