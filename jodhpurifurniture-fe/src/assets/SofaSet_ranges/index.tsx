...
const SofaSet_ranges = () => {
  ...
  return (
    <>
      <head>
        <SeoHeader
          title={getCetegory[0]?.meta_title}
          description={getCetegory[0]?.meta_description}
        />
      </head>
      <section className=" sec  section-2  row-remove">
        <div className="container2">
          <label className="top-xy range-other">Sofa Set Furniture </label>
          ...
          <Grid className=" row  boxes-padding ">
            ...
            <Slider {...settings}>
              {getCetegory &&
                getCetegory[0]?.child_categories &&
                getCetegory[0]?.child_categories.map((item) => {
                  return item.child_categories?.map((val) => {
                    return (
                      <div className="col-4 col-lg-2 boxes-padding h6 text-hover-image">
                        <Link
                          href={`/${getCetegory[0]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                          as={`/${getCetegory[0]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                        >
