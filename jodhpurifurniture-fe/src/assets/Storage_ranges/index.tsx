...
const Storage_ranges = () => {
  ...
  return (
    ...
    <head>
        <SeoHeader
          title={getCetegory[4]?.meta_title}
          description={getCetegory[4]?.meta_description}
        />
      </head>
      <section className="sec  section-2  row-remove">
        <div className="container2">
        <label className="top-xy range-other">Storage Furniture </label>
          ...
          {isLoading ? (
            ...
            <Slider {...settings}>
              {getCetegory &&
                getCetegory[4]?.child_categories &&
                getCetegory[4]?.child_categories.map((item) => {
                  return item.child_categories?.map((val) => {
                    return (
                      <div className="col-4 col-lg-2 boxes-padding h6 text-hover-image">
                        <Link
                         href={`/${getCetegory[4]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                         as={`/${getCetegory[4]?.slug_key}/${item.slug_key}/${val.slug_key}`}
                        >
                          <div className="image-box ">
                            <img src={val.image_url} alt={val.image_alt_tag}
                              title={val.image_title} />
                          </div>
                          <Typography className="widetext">{val.category_name}</Typography>
                        </Link>
                      </div>
                    );
                  });
